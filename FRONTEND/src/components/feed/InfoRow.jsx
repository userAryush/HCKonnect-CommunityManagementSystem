            ...
            <div className="bg-white rounded-standard border border-surface-border shadow-sm p-5">
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span
                            ...
                            style={{ width: '3px', height: '16px', background: 'var(--color-primary)' }}
                        />
                ...
                <div className="space-y-0.5">
                    {announcements.length > 0 ? announcements.map((ann, i) => (
                        <div
                            key={ann.id}
                            className="group flex items-start gap-3 px-3 py-2.5 rounded-xl transition-all cursor-pointer"
                            style={{ '--hover-bg': 'rgba(117,191,68,0.06)' }}
                            onMouseOver={e => e.currentTarget.style.background = 'rgba(117,191,68,0.07)'}
                            onMouseOut={e => e.currentTarget.style.background = 'transparent'}
                        >
                            {/* Dot with subtle green tint */}
                            <div
                                className="mt-1.5 flex-shrink-0 rounded-full"
                                style={{
                                    width: '6px', height: '6px',
                                    background: i === 0 ? 'var(--color-primary)' : 'var(--color-surface-border)',
                                    boxShadow: i === 0 ? '0 0 0 3px rgba(117,191,68,0.15)' : 'none',
                                }}
                            />
